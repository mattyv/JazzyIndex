...
- First 8 bytes: `uint64_t` count of elements
- Remaining bytes: sorted `uint64_t` values

### Generate RMI Code

For each 200M dataset (books, wiki, osm, fb), run:

```bash
cd external/RMI

# Books dataset
cargo run --release -- ../../benchmarks/datasets/books_200M.bin books_rmi linear,linear 100

# Wikipedia dataset
cargo run --release -- ../../benchmarks/datasets/wiki_200M.bin wiki_rmi linear,linear 100

# OSM dataset
cargo run --release -- ../../benchmarks/datasets/osm_200M.bin osm_rmi linear,linear 100

# Facebook dataset
cargo run --release -- ../../benchmarks/datasets/fb_200M.bin fb_rmi linear,linear 100
```

### Command Explanation

- `../../benchmarks/datasets/books_200M.bin` - Input dataset path
- `books_rmi` - Output name (generates `books_rmi.h` and `books_rmi.cpp`)
...
cargo run --release -- books_200M.bin books_rmi linear,linear 256

# Three layers with different branching factors
cargo run --release -- books_200M.bin books_rmi linear,linear,linear 10,100

# Cubic interpolation in first layer
cargo run --release -- books_200M.bin books_rmi cubic,linear 100
```
