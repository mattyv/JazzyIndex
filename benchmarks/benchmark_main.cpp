...
// Include RMI-generated headers for real-world datasets
// Generated by: cd external/RMI && cargo run --release -- <dataset> <name> linear,linear 100
#ifdef __has_include
...
#    define HAS_FB_RMI
#  endif
// Include RMI-generated headers for synthetic distributions
// Generated by: ./build/generate_rmi_datasets 10000
//           then: cd external/RMI && cargo run --release -- <dataset> <name> linear,linear 100
#  if __has_include("uniform_rmi.h")
#    include "uniform_rmi.h"
#    define HAS_UNIFORM_RMI
#  endif
#  if __has_include("exponential_rmi.h")
#    include "exponential_rmi.h"
#    define HAS_EXPONENTIAL_RMI
#  endif
#  if __has_include("clustered_rmi.h")
#    include "clustered_rmi.h"
#    define HAS_CLUSTERED_RMI
#  endif
#  if __has_include("lognormal_rmi.h")
#    include "lognormal_rmi.h"
#    define HAS_LOGNORMAL_RMI
#  endif
#  if __has_include("zipf_rmi.h")
#    include "zipf_rmi.h"
#    define HAS_ZIPF_RMI
#  endif
#  if __has_include("mixed_rmi.h")
#    include "mixed_rmi.h"
#    define HAS_MIXED_RMI
#  endif
#endif
#endif
...
// Register RMI benchmarks for synthetic distributions
void register_synthetic_rmi_benchmarks() {
    std::cout << "Registering RMI benchmarks for synthetic distributions..." << std::endl;

    // Helper to register RMI benchmarks for a synthetic distribution
    auto register_synthetic_rmi = [](const std::string& dist_name,
                                     const std::string& size_str,
                                     auto load_fn,
                                     auto lookup_fn,
                                     auto data_generator) {
        std::size_t size = std::stoull(size_str);
        const std::string base = "RMI/" + dist_name + "/N" + size_str;

        // Generate dataset once
        auto data_cache = std::make_shared<std::shared_ptr<std::vector<std::uint64_t>>>();

        auto load_once = [dist_name, size_str, data_cache, load_fn, data_generator]()
                         -> std::shared_ptr<std::vector<std::uint64_t>> {
            if (!*data_cache) {
                std::cout << "Loading " << dist_name << " dataset for RMI benchmarks..." << std::endl;
                *data_cache = std::make_shared<std::vector<std::uint64_t>>(data_generator());

                // Load RMI model data
                if (!load_fn("rmi_data")) {
                    std::cerr << "Warning: Failed to load RMI model for " << dist_name << std::endl;
                }
            }
            return *data_cache;
        };
    ...
    // Register for each available synthetic RMI (must match dataset size used during generation)
    const std::string dataset_size = "10000";  // Default size from generate_rmi_datasets
...
#ifdef ENABLE_RMI_BENCHMARKS
        // Register RMI comparison benchmarks for synthetic distributions
        register_synthetic_rmi_benchmarks();
#endif
    }
